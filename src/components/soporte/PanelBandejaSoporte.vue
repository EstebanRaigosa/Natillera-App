<template>
  <div class="flex min-h-0 flex-1 flex-col">
    <!-- ── Filtros y búsqueda ── -->
    <!-- Con una conversación abierta en una sola columna, los filtros estorban:
         ocupan el alto que necesita el hilo y filtran una lista que no se ve. -->
    <div
      v-if="!idActivo || (esEscritorio && !compacto)"
      :class="['flex flex-wrap items-center gap-2', compacto ? 'px-3 pt-3 pb-2' : 'mb-3']"
    >
    ...
    >
      ...
      <section
        :class="[
          ...
          idActivo ? 'flex' : (compacto ? 'hidden' : 'hidden lg:flex'),
        ]"
      >
...
<script setup>
/**
 * Bandeja del soporte: lista de conversaciones de todos los usuarios, hilo y
 * herramientas de gestión (estado, nota interna, eliminar).
 *
 * Vive como componente y no dentro de la vista por el mismo motivo que
 * `PanelConversaciones`: hay dos sitios donde se usa exactamente lo mismo —el
 * modal que abre el botón flotante y la página `/admin/soporte`, que sigue
 * existiendo porque las notificaciones apuntan a una URL concreta—. Duplicarlo
 * significaría mantener dos bandejas.
 *
 *   `compacto`  → una sola columna con navegación bandeja ⇄ hilo (modal)
...
import EliminarConversacionModal from './EliminarConversacionModal.vue'
...
const props = defineProps({
  ...
  /** Conversación a abrir al montar cuando no se usa la ruta (notificaciones). */
  conversacionInicial: { type: String, default: null },
})

const emit = defineEmits(['estado-canal'])
...
const idActivo = ref(props.usarRuta
  ...
  : (props.conversacionInicial ?? soporte.conversacionAbiertaBandeja))
const borrador = ref('')
...
watch(estadoCanal, (valor) => emit('estado-canal', valor), { immediate: true })
...
async function abrirConversacion(id, { navegar = true } = {}) {
  idActivo.value = id
  soporte.conversacionAbiertaBandeja = id
  borrador.value = ''
...
function cerrarConversacion() {
  idActivo.value = null
  soporte.conversacionAbiertaBandeja = null
  if (props.usarRuta) router.push('/admin/soporte')
}
