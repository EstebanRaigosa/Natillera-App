<template>
  <div class="max-w-7xl lg:max-w-6xl xl:max-w-7xl mx-auto space-y-3 sm:space-y-5 lg:space-y-6 overflow-x-hidden relative">
      ...
      >
        ...
        <div v-else-if="detalleSancionesAbierto" class="space-y-4">
          ...
          <template v-else>
            ...
            <!-- Sanciones por retiro de socio: registradas al desactivar a un socio.
                 Aparecen aquí porque también suman al indicador "Sanciones" (utilidades_clasificadas). -->
            <div
              v-if="(detalleSanciones.sancionesRetiro || []).length > 0"
    ...
    <!-- Modal: Todos los movimientos con filtro día/semana/mes/todos -->
    <ModalWrapper
      :show="!!modalTodosMovimientos"
      :z-index="50"
      align="bottom"
      :ios-soft-backdrop="true"
      overlay-class="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4"
    ...
    >
      ...
      <div class="sm:hidden flex-shrink-0 bg-[#1B5E37]">
        <div class="flex items-center gap-3 pl-4 pr-3 pb-3 pt-[max(0.75rem,env(safe-area-inset-top))] min-h-[4.2rem]">
          ...
          <div class="min-w-0 flex-1 py-0.5 pr-1 text-left">
            <h3 class="font-display text-base font-bold leading-tight text-white">Todos los movimientos</h3>
            <p class="mt-0.5 text-[0.6875rem] font-normal leading-snug text-white/85">Filtra por período</p>
          </div>
          <button
            type="button"
            class="flex h-11 w-11 shrink-0 items-center justify-center rounded-full text-white/95 transition-colors hover:bg-white/15 active:bg-white/25 [-webkit-tap-highlight-color:transparent] touch-manipulation"
            aria-label="Cerrar"
            @click="cerrarModalTodosMovimientos"
          >
            <XMarkIcon class="h-6 w-6" />
          </button>
        </div>
      </div>
      <!-- Cabecera desktop: icono centrado + títulos + X flex -->
      <div class="hidden sm:block flex-shrink-0 bg-[#1B5E37]">
        <div class="flex items-start pt-[max(0.5rem,env(safe-area-inset-top))] pl-4 pr-3 pb-5">
          ...
          <div class="flex min-w-0 flex-1 flex-col items-center px-2 text-center">
            ...
            <p class="mt-1 text-xs font-normal text-white/85">Filtra por período</p>
          </div>
          <button
            type="button"
            class="flex h-11 w-11 shrink-0 items-center justify-center rounded-full text-white/95 transition-colors hover:bg-white/15 active:bg-white/25 [-webkit-tap-highlight-color:transparent] touch-manipulation"
          ...
          <div class="flex items-center gap-2 overflow-x-auto">
            ...
            <div class="flex items-center gap-2">
              <button
                ...
                :key="f.value"
                type="button"
                @click="filtroMovimientosPeriodo = f.value"
                :class="[
    ...
    >
          ...
          <div class="flex-shrink-0 px-6 pt-2 pb-[max(1.25rem,env(safe-area-inset-bottom))] space-y-3 bg-white">
            <button
              ...
              class="btn-modal-primary inline-flex w-full items-center justify-center gap-2"
            >
...
const modalTodosMovimientos = ref(false)
const filtroMovimientosPeriodo = ref('todos') // 'dia' | 'semana' | 'mes' | 'todos'
...
useBodyScrollLock(modalTodosMovimientos)
...
/**
 * Carga movimientos extra para «Últimos movimientos».
 * @param {string} natilleraId
 * @param {string[]|null} socioNatilleraIdsPrecargados Si vienen de `fetchNatillera`, se evita 1 round-trip a `socios_natillera`.
 */
async function cargarMovimientosRecientesNatillera(natilleraId, socioNatilleraIdsPrecargados = null) {
  if (!natilleraId) {
    movimientosNatilleraExtra.value = []
    return
  }
  try {
    let socioNatilleraIds = Array.isArray(socioNatilleraIdsPrecargados)
      ? socioNatilleraIdsPrecargados.filter(Boolean)
      : null
    if (!socioNatilleraIds || socioNatilleraIds.length === 0) {
      const { data: snRows, error: errSn } = await supabase
        .from('socios_natillera')
        .select('id')
        .eq('natillera_id', natilleraId)
      if (errSn) throw errSn
      socioNatilleraIds = (snRows || []).map(r => r.id)
    }
    const out = []
...
const todosMovimientos = computed(() => {
  const items = []
  ...
  for (const c of cuotas) {
    ...
    items.push({
      ...
      formaPago: (c.tipo_pago || c.forma_pago || (esCuotaPagadaCompleta || esCuotaParcial ? 'efectivo' : '')).toString(),
    })
  }
...
const ultimosMovimientos = computed(() =>
  todosMovimientos.value.slice(0, 6).map(({ sortAt: _s, ...rest }) => rest)
)
...
const movimientosFiltrados = computed(() => {
  const lista = todosMovimientos.value
  const filtro = filtroMovimientosPeriodo.value
  ...
  if (filtro === 'dia') {
    ...
    // Lunes de la semana actual (ISO-like, compatible con es-CO)
    const dow = (ahora.getDay() + 6) % 7 // 0 = lunes
    desde = new Date(ahora.getFullYear(), ahora.getMonth(), ahora.getDate() - dow)
  } else if (filtro === 'mes') {
    desde = new Date(ahora.getFullYear(), ahora.getMonth(), 1)
  }
  const desdeTs = desde.getTime()
  return lista.filter(m => {
    const ts = _tsOrdenMovimiento(m.sortAt)
    return Number.isFinite(ts) && ts >= desdeTs
  })
})
...
function abrirModalTodosMovimientos() {
  filtroMovimientosPeriodo.value = 'todos'
  modalTodosMovimientos.value = true
}

function cerrarModalTodosMovimientos() {
  modalTodosMovimientos.value = false
}
...
async function cargarDetalleSanciones() {
  ...
  detalleSanciones.value = { loading: true, lista: [], sancionesRetiro: [], totalRetiro: 0 }
  try {
    ...
    // Sanciones por retiro de socio: se cargan en paralelo a las multas de cuotas.
    // Provienen de utilidades_clasificadas con tipo='sanciones' y descripción específica
    // (sanción al desactivar socio o reversión por reactivación). NO están en la tabla cuotas,
    // por eso se sumaban al indicador pero no aparecían en este detalle.
    const utilSancionesPromise = supabase
      .from('utilidades_clasificadas')
    ...
    const cuotasPromise = snIds.length > 0
      ? supabase
          .from('cuotas')
    ...
    const [cuotasRes, utilRes] = await Promise.all([cuotasPromise, utilSancionesPromise])
    if (cuotasRes.error) throw cuotasRes.error

    const filas = cuotasRes.data || []
    ...
    // Ordenar por año-mes asc y valor desc dentro del mismo período
    lista.sort((a, b) => {
      if (a.anio !== b.anio) return a.anio - b.anio
    ...
    // Procesar sanciones por retiro (agrupadas por socio, restando reversiones)
    const sancionesRetiroPorSocio = {}
    const filasUtil = utilRes?.data || []
    filasUtil.forEach(r => {
      const desc = (r.descripcion || '').toString()
      const monto = parseFloat(r.monto) || 0
      const matchDirecto = desc.match(/^Sanci[oó]n por retiro:\s*(.+)$/i)
      const matchReversion = desc.match(/^Reversi[oó]n reactivaci[oó]n\s*-\s*Sanci[oó]n por retiro:\s*(.+)$/i)
      let nombreSocio = null
      if (matchReversion) nombreSocio = matchReversion[1].trim()
      else if (matchDirecto) nombreSocio = matchDirecto[1].trim()
      if (!nombreSocio) return
      sancionesRetiroPorSocio[nombreSocio] = (sancionesRetiroPorSocio[nombreSocio] || 0) + monto
    })
    const sancionesRetiro = Object.entries(sancionesRetiroPorSocio)
      .filter(([, v]) => v > 0)
      .map(([socio, valor]) => ({ socio, valor }))
      .sort((a, b) => (b.valor || 0) - (a.valor || 0))
    const totalRetiro = sancionesRetiro.reduce((s, it) => s + (it.valor || 0), 0)

    detalleSanciones.value = { loading: false, lista, sancionesRetiro, totalRetiro }
  } catch (e) {
    console.error('Error cargando detalle sanciones:', e)
    detalleSanciones.value = { loading: false, lista: [], sancionesRetiro: [], totalRetiro: 0 }
  }
}
...
watch(() => id.value, async (newId, oldId) => {
  ...
  if (newId && newId !== oldId) {
    ...
    try {
      configStore.cargarConfiguracion().catch(() => {})
      ...
      const idsSociosSn = natilleraResult?.socios_natillera?.map((s) => s.id).filter(Boolean) ?? []

      // Ocultar pantalla de carga en cuanto hay natillera + cuotas (sin esperar nextTick)
      cargandoNatillera.value = false
      ...
      Promise.all([
        ...
        cargarMovimientosRecientesNatillera(newId, idsSociosSn)
...
onMounted(async () => {
  ...
  try {
    ...
    const backgroundTasks = [
      ...
      cargarMovimientosRecientesNatillera(natilleraId, idsSociosSnMount)
    ]
